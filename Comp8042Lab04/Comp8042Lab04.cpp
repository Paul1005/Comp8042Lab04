		...
		if (p.empty()) {
			...
			for (int i = p.size() - 1; i >= 0; i--) {
				...
				if (!p[i]->balanced()) {
					...
					int rightHeight = -1;
					if (p[i]->right != NULL) {
						rightHeight = p[i]->right->height;
					}

					int leftHeight = -1;
					if (p[i]->left != NULL) {
						leftHeight = p[i]->left->height;
					}

					int balanceFactor = leftHeight - rightHeight;
					if (balanceFactor > 1 && x < p[i]->left->value) {
						p[i] = p[i]->singleRightRotation();
					}
					else if (balanceFactor < -1 && x > p[i]->right->value) {
						p[i] = p[i]->singleLeftRotation();
					}
					else if (balanceFactor > 1 && x > p[i]->left->value) {
						p[i] = p[i]->doubleLeftRightRotation();
					}
					else if (balanceFactor < -1 && x < p[i]->right->value) {
						p[i] = p[i]->doubleRightLeftRotation();
					}
			...
			//TODO this line is just to prevent the warning: control may reach end of non-void function``
			return false;
		}
	}
